...
1. 모델링 
=> 생각 하는 바를 글과 그림으로 표현하는 것

2. 렌더링
=> 명령어를 분석하여 화면에 그리는 것.
=> ex) HTML 렌더러(Renderer) : HTML을 분석하여 웹브라우저 화면에 출력하는 것

3. DB모델링
=> 데이터의 구조를 글과 그림을 표현
=> 데이터 사이의 관계를 쉽계 파악할 수 있다.
=> 데이터의 중복을 제거하는 것.
* 데이터 중복의 문제점
    1) 데이터 동기화가 어렵다.
        => 데이터가 변경되었을 떄 그 변경 사항을 빠뜨리기 쉽다.
        => 업무에 영향을 미친다.
        => 예) 주소 변경, 전화번호 변경
    2) 메모리 낭비
4. DB모델링 절차
...
2) 엔티티에 속성을 선언한다.
    => 보통 'column'이라 부른다.
3) 제 1 정규화
    => 중복 칼럼이나 중복 데이터가 있ㅎ다면 별도의 테이블로 분리.
    => 기존 테이블과 부모 자식 관계를 맺는다.
4) 제 2 정규화
    => primary key 가 두 개 이상일 때 수행한다.
    => 모든 일반 컬럼은 PK에 종속되어야 한다.
    => PK에 종속되지 않는 컬럼이 있으면 별도의 테이블로 분리한다.
    => 기존 테이블과 부모 자식관계를 맺는다.
5) 제 3 정규화
...
9) 인덱스 대상 컬럼을 지정한다.
    ...
    => 인덱스 컬럼이 많으면 해당 테이블에 값을 넣거나 변경, 삭제하는 속도가 느려진다.
    이유? 추가, 변경, 삭제할 때 마다 색인표를 새로 만들어야 하기 때문이다.
    => PK나 UNIQUE 컬럼은 자동으로 인덱스가 생성되어 따로 지정할 필요가 없다.
10) default 값을 지정한다.
    => 값을 생략했을 때 기본으로 설정 될 값을 지정한다.
...
4) 포워드 엔지니어링(forward engineering) 수행
    => 모델링 결과 ====> SQL 문 변환
* cf) 리버스 엔지니어링(reverse engineering) => DBMS 테이블 정보 ====> 모델링 생성.
...
신청자,전화  <--  unique 컬럼
=> 주의사항
    1) PK컬럼은 다른 테이블에서 참조할 수 있기 때문에, 변경할 수 없다.

4) 만약 적절한 PK를 찾을 수 없다면,
=> 일련번호나 특정 형쇼기의 표현하는 코드 값을 PK로 선정한다.

[Foreign key 컬럼]
=> 다른 테이블의 데이터를 참조하기 위해 그 테이블의 PK값을 저장하는 컬럼
=> 그래서 이름이 "외부 키"라고 부른다.
