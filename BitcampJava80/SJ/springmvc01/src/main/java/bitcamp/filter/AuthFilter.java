...
public class AuthFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    String sPath = request2.getServletPath();
    if (!sPath.startsWith("/controller31/")) {// 로그인 요청이 아니라면,
      // 무조건 로그인 되어있는지 검사해야 한다.
...
/*
 * # 로그인 여부 검사하기 - 필터를 사용하지 않으면 모든 서블릿에 로그인 여부를 검사하는 코드를 삽입해야 한다.
 */