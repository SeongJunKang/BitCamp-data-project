...
- 상속한다고 명시하지 않으면 컴파일러가 자동으로 삽입.
- 자바 클래스가 기본으로 가져야할 필드 (변수)와 메서드를 구비하고 있다.
- 주요 메서드
1.  toString() : 클래스이름@해시코드값
2.  hashCode() : 인스턴스를 구별하기 위한 4바이트 정수값.
...
6.  finalize() : 가비지 컬렉터가 쓰레기 인스턴스를 해제하기 전에 호출한다.
                 ...
                 쓰레기 인스턴스가 언제 수거되는지 알 수 없다. 따라서, finalize()의
                 호출시점을 명확히 알 수 없다. 메모리가 부족하지 않다면, 프로그램이
                 종료될 때 까지, 가비지 컬렉터가 동작하지 않는다. --->프로그램이 종료
                 될 떄까지 finalize()가 호출되지 않을 수 있다.
                 설명은 했지만 오버라이드나 의존하는 코드를 작성하지말라..
*/
