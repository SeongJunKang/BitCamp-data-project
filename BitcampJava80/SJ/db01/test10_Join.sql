...
=> n * m 조인. 두결과 데이터를 1:1로 연결한다.
=> 조인 전
SELECT LNO,TITL FROM LECTS;
SELECT RNO,RNM FROM ROOMS;

=> 1:1 크로스 조인
SELECT LNO,TITL,RNO,RNM FROM LECTS, ROOMS; /*RNO가 불분명하다. LECTS , ROOMS 에도 존재하기 때문*/
SELECT LNO,TITL,LECTS.RNO,RNM FROM LECTS, ROOMS; /*RNO를 명확히 해줘야한다.*/
SELECT LNO,TITL,RNM FROM LECTS, ROOMS;

=> 최신 문법
SELECT LNO,TITL,LECTS.RNO,RNM FROM LECTS CROSS JOIN ROOMS;

=> 테이블 별명 사용
SELECT LNO,TITL,L.RNO,RNM FROM LECTS L CROSS JOIN ROOMS R;
...
=> 조건 검색시 테이블 이름을 사용하면 불편하다. => 별명 사용하라!
...
4.JOIN ~ ON 조건
=> 두 테이블에 조인할 때 사용할 컬럼의 이름이 같지 않을 때.
SELECT LNO,TITL,L.RNO,RNM
...
=> 두 개 테이블을 조인 할 떄 어느 쪽 테이블을 기준해야 할 것인지 적절치 않다.
...
=> 교육과정 - 강의실 - 강사번호
SELECT L.LNO,TITL,L.RNO,RNM,T.TNM
FROM LECTS L LEFT OUTER JOIN ROOMS R ON L.RNO=R.RNO
